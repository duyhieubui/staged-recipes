...
$BUILD_PREFIX/bin/cmake -B build  -DCMAKE_INSTALL_PREFIX=$PREFIX 
$BUILD_PREFIX/bin/cmake --build build -j $CPU_COUNT --target install
...
$BUILD_PREFIX/bin/cmake -B build  -DBUILD_DEPS:BOOL=ON \
      ...
      .
$BUILD_PREFIX/bin/cmake --build build --config Release -j $CPU_COUNT --target install

## there is a bug with SPDLOG 1.11 and fmt 0.9
## disable SPDLOG_FMT_EXTERNAL
cd $SRC_DIR
$BUILD_PREFIX/bin/cmake -B build -DTCL_LIB_PATHS="$BUILD_PREFIX;$PREFIX" \
      -DCMAKE_FIND_ROOT_PATH="$BUILD_PREFIX;$PREFIX" \
